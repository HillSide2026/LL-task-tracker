...
- Host the matter management app and its associated workflows at `firm.levinellp.ca`.
...
- Use staging domains that separate frontend hosting from backend services:
  - `staging-firm.levinellp.ca` for the Vercel-hosted React portal
  - `staging-api-firm.levinellp.ca` for backend routes such as `/auth`, `/api`, and `/storage`
  - `staging-s3.firm.levinellp.ca` for browser-facing object storage URLs
...
- Configure Vercel production environment variables with `REACT_APP_*` values. The frontend now prefers build-provided `REACT_APP_*` values and falls back to Docker-oriented runtime placeholders when those values are absent.
