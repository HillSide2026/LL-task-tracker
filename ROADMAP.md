...
- Host the matter management app and its associated workflows at `matters.levinellp.ca`.
...
- Use staging domains that separate frontend hosting from backend services:
  ...
  - `staging-matters.levinellp.ca` for backend routes such as `/auth`, `/api`, and `/storage`
  - `staging-s3.matters.levinellp.ca` for browser-facing object storage URLs
