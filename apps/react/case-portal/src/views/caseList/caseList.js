...
import { ADMIN_OPENING_CONTROL_CASE_DEF_ID } from 'common/adminLifecycle'
...
export const CaseList = ({ status, caseDefId, filters = {} }) => {
  ...
  const [filter, setFilter] = useState({
    sort: 'desc',
    limit: filters.exceptionOnly ? 100 : 10,
    after: '',
    before: '',
    cursors: {},
    hasPrevious: false,
  ...
  }, [caseDefId, status, openNewCaseForm, JSON.stringify(filters)])
  ...
  const makeColumns = () => {
    ...
    if (caseDefId === ADMIN_OPENING_CONTROL_CASE_DEF_ID) {
      columns.push(
        {
          field: 'adminStateDescription',
  ...
  const handlerNextPage = () => {
    ...
    CaseService.filterCase(keycloak, caseDefId, status, next, filters)
      .then((resp) => {
  ...
  const handlerPriorPage = () => {
    ...
    CaseService.filterCase(keycloak, caseDefId, status, prior, filters)
      .then((resp) => {
        const { data, paging } = resp
